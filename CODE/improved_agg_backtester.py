...
def backtest_pairs(pairs, all_data, initial_capital, max_pairs):
    ...
    for ticker1, ticker2, correlation in pairs:
        ...
        # Instead of just storing open PnL, store cumulative value of the pair strategy.
        pair_cumulative = pd.Series(0.0, index=data.index)
        
        # Keep track of yesterday's total position value to compute daily changes
        # We'll consider the "value" of this pair as cumulative profit line starting from 0.
        last_value = 0.0
        ...
        for i in range(window, len(z_score)):
            ...
            # If no open position, consider entry signals
            if not has_open_position:
                if (z_score.iloc[i] > enter1 and z_score.iloc[i] > 0):
                    # Enter trade: short ticker1, long ticker2
                    ...
                    # Enter trade: long ticker1, short ticker2
                    shares1 = (0.5 * capital_per_pair) / data[ticker1].iloc[i]
                    shares2 = (0.5 * capital_per_pair) / data[ticker2].iloc[i]
                    position1 = 'long'
                    position2 = 'short'
            ...
            if has_open_position:
                ...
                # Current total value = last_value + today's change
                # The change is (current_pnl) - (yesterday's open pnl).
                # But since we keep a running total, just do:
                # new_value = realized_so_far + current_pnl (open)
                # At this point, realized_so_far is included in last_value if we updated at close-outs.
                
                # For now, since no trade closed today, cumulative stays the same except for the open PnL:
                # Actually, we track daily increments only. last_value was the last day's cumulative value.
                # Today's value = last_value (yesterday's cumulative) + (today's open PnL change).
                # But we need to be careful. We have no realized changes yet, just an open position.
                # For a truly cumulative series, let's say:
                new_value = (total_profit) + current_pnl  # total_profit + open PnL
                pair_cumulative.iloc[i] = new_value
            else:
                # No open position: value stays at realized total_profit
            ...
            if in_trade1 and (z_score.iloc[i] < exit_threshold):
                ...
                in_trade1 = False
                open_trade = None
                # Update cumulative to reflect realized profit
                pair_cumulative.iloc[i] = total_profit
        ...
        # At this point, pair_cumulative is a cumulative value series for this pair.
        # Even if no trades were made, it remains at 0.

        # Convert cumulative series to daily increments
        final_series = convert_to_daily_increments(pair_cumulative)
    ...
    if not portfolio_df.empty:
       # Sum across all pairs to get daily increments (not cumulative PnL!)
        portfolio_df['Daily_Portfolio_PnL'] = portfolio_df.sum(axis=1)

        # Now convert daily increments to a cumulative PnL
        portfolio_df['Cumulative_PnL'] = portfolio_df['Daily_Portfolio_PnL'].cumsum()
        ...
        # Now resample to daily frequency and compute returns
...
pairs_file_path = 'pairsfor2023_2024.txt' # Input pairs
data_file = r'C:\Users\admin\Documents\z_score_code\all_stock_data2023_10_08to2024_12_06polymulti.csv'
...
if all_data is not None:
    ...
    portfolio_df.to_excel('portfolio_mtm_values_2023_10_08to2024_12_06.xlsx')
    print("Saved portfolio MTM spreadsheet to 'portfolio_mtm_values_2023_10_08to2024_12_06.xlxs'")

    daily_returns.to_csv('daily_returns2023_2024.csv', header=['Daily_Return'])
...
if not daily_returns.empty:
    ...
    daily_returns = daily_returns.iloc[40:-28]
